...
if(isset($_GET['query']))
{
    ...
    $search = $_GET['search'];
    if($_GET['advancedSearch']=="people")
    {
        $result = getSearchResults($search,$conn);

    }
    else
    {
        $result = getQuestionSearchResults($search,$conn);
        $flag = 1;
    }
         ...
         <div class="container-fluid">
            <div class="news-feed">
                ...
                <div class="container" style="margin-left: 70px;">
                    <p style="font-size: 20px;">Showing <?php echo $numOfResult ?> results for "<?php echo $search?>"</p>
                                ...
                                <?php
                                    if($numOfResult>0)
                                    {
                                        if($flag==0)
                                        {
                                            ...
                                            {
                                                searchDisplay($_SESSION['rollNo'],$row['rollNo'],$row['firstName']." ".$row['lastName'],$row['userPic'],$conn);
                                            }
                                        }
                                        else
                                        {
                                            while ($row = mysqli_fetch_assoc($result))
                                            {
                                                searchQuestionDisplay($row['rollNo'],$row['questionId'],$row['heading'],$conn);
                                            }
                                        }
                                    }
